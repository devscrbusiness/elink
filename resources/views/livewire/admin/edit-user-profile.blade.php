<x-admin.layout :user="$user" :business="$user->business" :isAdminEditing="true">
    <div class="p-6 bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_profile_title') }}</h2>
        <p class="mt-2 text-gray-600 dark:text-gray-300">
            {{ __('admin.edit_profile_subtitle') }}
        </p>
        ...
        <form wire:submit="updateUserProfile" class="my-6 w-full space-y-6">
            <flux:input wire:model="name" :label="__('app.name')" type="text" required />
            <flux:input wire:model="email" :label="__('app.email')" type="email" required />
