<div class="p-6 bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700">
    ...
    <div class="mt-8">
        <!-- Encabezados -->
        <div class="hidden md:grid grid-cols-12 gap-4 px-4 py-2 rounded-full text-sm font-bold bg-blue-600 text-white uppercase tracking-wider border-b border-gray-200 dark:border-zinc-700">
            ...
            <div class="col-span-2">{{ __('admin.table_header_business') }}</div>
            <div class="col-span-3 text-right">{{ __('admin.table_header_actions') }}</div>
        </div>
        ...
        <div class="space-y-2 mt-4">
            @forelse ($users as $user)
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-lg bg-gray-50 hover:bg-gray-100 dark:bg-zinc-900/50 dark:hover:bg-zinc-700/50 transition-colors">
                    ...
                    <!-- Empresa -->
                    <div class="md:col-span-2 text-sm text-gray-500 dark:text-gray-300 truncate"><span class="md:hidden font-semibold text-gray-600 dark:text-gray-400">{{ __('admin.business_label') }} </span>{{ $user->business->name ?? 'N/A' }}</div>

                    <!-- Acciones -->
                    <div class="md:col-span-3 flex items-center justify-start md:justify-end space-x-2">
                        @if($user->business)
